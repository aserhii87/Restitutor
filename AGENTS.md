...
- Don't worry about formatting when writing code. You can output condensed code without white space if it is preferred - the project's formatter (`pnpm run check` in root folder) will handle formatting. If unrelated files change after running the formatter, leave those changes be.
